...
class AppForm(npyscreen.FormMuttActiveTraditionalWithMenus):
    ...
    def sendHandler(self, event):
        ...
        self.parentApp.message_queue.put(
            {"state": self.parentApp.state, "currentSend": cur}
        )
...
class SelectForm(npyscreen.Form):
    ...
    def create(self):
        ...
        td = npyscreen.TreeData(
            content="Select one:", selectable=False, ignore_root=False
        )
...
class AppState(object):
    ...
    def __str__(self):
        return "state type: {} name: {} numbers: {}".format(
            self.convType, self.toName, ", ".join(self.numbers)
        )
    ...
    def numbers(self):
        return (
            [self.toNumber] if self.is_user else self.toNumbers if self.is_group else []
        )
    ...
    def toName(self):
        return (
            self.user["name"]
            if self.is_user
            else (self.group["name"] if self.is_group else None)
        )
...
class SignalApp(npyscreen.StandardApp):
    ...
    def addEnvelope(self, env):
        ...
        self.app.wMain.addDatedValues([gen_line])
        ...
        return Envelope.load(
            {
                "envelope": {
                    ...
                    "dataMessage": {"timestamp": now, "message": msg},
                }
            },
            self,
            Envelope.SELF,
    ...
    def handleEnvelope(self, env):
        ...
        if env.timestamp and (time.time() - env.epoch_ts) >= 60:
            log(
                "ignoring envelope due to time difference of",
        ...
        if env.callMessage.is_busy():
            ...
            npyscreen.notify_wait(
                "The caller is busy", title="Call from {}".format(env.source)
            )
        ...
        if env.callMessage.is_hangup():
            ...
            npyscreen.notify_wait(
                "The caller hung up", title="Call from {}".format(env.source)
            )
...
class Envelope(object):
    ...
    def should_display(self, toNumber, phone):
        return (
            (self.source == toNumber) or (self.source == phone)
        ) and self.dataMessage.should_display()
    ...
    def should_notify(self, toNumber, phone):
        ...
        return (
            (self.source != toNumber)
            and (self.source != phone)
            and self.dataMessage.should_notify()
        )
    ...
    def gen_line(self):
        if self.sourceName:
            return (
                self.format_ts(),
                "{} ({})".format(self.sourceName, self.source),
                self.dataMessage.gen_line(),
            )
        ...
        for msg in self.readMessages:
            if env and env.dataMessage.is_message():
                ...
                ret = ret or (
                    ...
                    and self._compare_ts(
                        env.dataMessage.timestamp, msg.get("timestamp")
                    )
                )
...
class SignalMessageThread(threading.Thread):
    ...
    def run(self):
        ...
        if not self.signal:
            log("dbus err")
            npyscreen.notify_wait(
                "Unable to get signal {} bus. Messaging functionality will not function.".format(
                    self.app.state.bus
                ),
                title="Error in SignalDaemonThread",
            )
            exit(1)
        log("got dbus")
...
class SetupLinkPromptForm(npyscreen.Form):
    ...
    def prompt(self):
        ...
        npyscreen.notify_confirm(
            "Couldn't open your signal config file for:\nPhone: {}\nConfig dir: {}".format(
                state.phone, state.configDir
            )
            + "\nDo you want to link a new device right now? Hit Tab-Enter to continue, or Ctrl+C",
            title="No signal-cli config",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bus",
        dest="bus",
        help="DBus session type (default: session)",
        default="session",
    ...
    parser.add_argument(
        ...
        dest="configDir",
        help="Config folder",
        default="{}/{}".format(pathlib.Path.home(), CONFIG_FOLDER),
    )
